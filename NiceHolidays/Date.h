...
inline unsigned daysYear(unsigned year);									// Returns the number of days of a year (365 or 366)
inline unsigned abs(const Date date);										// Returns the number of days from the epoch to the date
inline Date date(unsigned days);											// Calculates the date from the number of days from epoch