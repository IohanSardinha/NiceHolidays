...
unsigned viewPackets(Agency agency, unsigned origin)
{
	...
	for (Packet p : agency.getPackets())
	{
		...
		packets.push_back(vector<string>({p.getId(),sites,str(p.getBeginDate()),str(p.getEndDate()),to_string(p.getPricePerPerson()),to_string(p.getSoldPlaces()),to_string(p.getMaxPlaces())}));
	}
